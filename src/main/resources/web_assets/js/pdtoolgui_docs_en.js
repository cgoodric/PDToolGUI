...
var TOOLTIPS = {
    "config_dep_conf" : {
        ...
        "label[for=property_order_precedence]":              "Comma separated list of values indicating the order of precedence of variable sources (<span class='code'>JVM</span>, <span class='code'>SYSTEM</span>, or <span class='code'>PROPERTIES_FILE</span>) to use when referencing variables.",
        ...
        "label[for=vcs_base_folder_init_add]":               "Command to execute to add a folder after initializing the VCS workspace. Add Options:<br/><b>TFS:</b> <span class='code'>tf.cmd add ${fullResourcePath} -recursive -noprompt /login:${VCS_USERNAME},${VCS_PASSWORD} ${VCS_OPTIONS} ${VCS_BASE_FOLDER_INIT_ADD}<br/>&nbsp;&nbsp;&nbsp;&nbsp;</span>e.g. <span class='code'>E:/dev/vcs/TEE-CLC-11.0.0/tf.cmd add P:/TFSww/Composite_62/cis_objects/shared/test00  $/Composite_62/cis_objects -noprompt /login:user,********</span><br/><b>SVN:</b> <span class='code'>svn add ${fullResourcePath} ${SVN_AUTH} ${VCS_OPTIONS} ${VCS_BASE_FOLDER_INIT_ADD}</span><br/><b>P4:</b> <span class='code'>p4 add ${fullResourcePath} ${VCS_BASE_FOLDER_INIT_ADD}</span><br/><b>CVS:</b> <span class='code'>cvs add ${fullResourcePath} ${VCS_BASE_FOLDER_INIT_ADD}</span>",
        ...
        "label[for=vcs_cis_import_options]":                 "Options to use when importing resources into CIS. Occurs during a vcsCheckout when the temporary checkout.car file is imported into the target server. The import is the final step in the process.  The backwards compatible option is to include access: -includeaccess. Consult the Archive Module documentation for more options.",
        "label[for=vcs_cis_export_options]":                 "Options to use when exporting resources from CIS. Occurs during a vcsCheckin when the temporary checkout.car file is exported from the target server. The export from the target is used to compare resources against the VCS repository.  The backwards compatible option is to leave this option blank. Consult the Archive Module documentation for more options.",
    ...
    "config_servers" : {
        ...
        "label[for=allowVariables]":                         "Indicates whether variable references are allowed in all server entry fields"
    },
    "config_vcs_init" : {
    ...
    "module_vcs" : {
        ...
        "label[for=vcsBaseFolderInitAdd]":                   "Command to execute to add a folder after initializing the VCS workspace. Add Options:<br/><b>TFS:</b> <span class='code'>tf.cmd add ${fullResourcePath} -recursive -noprompt /login:${VCS_USERNAME},${VCS_PASSWORD} ${VCS_OPTIONS} ${VCS_BASE_FOLDER_INIT_ADD}<br/>&nbsp;&nbsp;&nbsp;&nbsp;</span>e.g. <span class='code'>E:/dev/vcs/TEE-CLC-11.0.0/tf.cmd add P:/TFSww/Composite_62/cis_objects/shared/test00  $/Composite_62/cis_objects -noprompt /login:user,********</span><br/><b>SVN:</b> <span class='code'>svn add ${fullResourcePath} ${SVN_AUTH} ${VCS_OPTIONS} ${VCS_BASE_FOLDER_INIT_ADD}</span><br/><b>P4:</b> <span class='code'>p4 add ${fullResourcePath} ${VCS_BASE_FOLDER_INIT_ADD}</span><br/><b>CVS:</b> <span class='code'>cvs add ${fullResourcePath} ${VCS_BASE_FOLDER_INIT_ADD}</span>",
        ...
        "label[for=vcsCisImportOptions]":                    "Options to use when importing resources into CIS. Occurs during a vcsCheckout when the temporary checkout.car file is imported into the target server. The import is the final step in the process.  The backwards compatible option is to include access: -includeaccess. Consult the Archive Module documentation for more options.",
        "label[for=vcsCisExportOptions]":                    "Options to use when exporting resources from CIS. Occurs during a vcsCheckin when the temporary checkout.car file is exported from the target server. The export from the target is used to compare resources against the VCS repository.  The backwards compatible option is to leave this option blank. Consult the Archive Module documentation for more options.",
